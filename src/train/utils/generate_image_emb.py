...
class SIGLIP_CLASSIFIER(CLIP):
    ...
    def forward(self, image_tensor, input_ids, attention_mask=None, labels=None, alpha=0.5, beta=0.5): 
        ...
        # sim_labels = 2*torch.eye(logits.shape[0]).to(logits.device) - torch.ones(logits.shape).to(logits.device)
        ...
        self_similarity = ztxt @ ztxt.T
        sim_loss = -torch.sum(F.logsigmoid(self_similarity*logits)) / logits.shape[0]
        
        # Calculating the Classification Loss
        mask = ~torch.isnan(labels)
        img_class_logits = self.classifier(image_embeddings)
        img_class_loss = F.binary_cross_entropy_with_logits(
            img_class_logits[mask],
            labels[mask],
            reduction='sum')
        txt_class_logits = self.classifier(text_embeddings)
        ...
        return total_loss, (sim_loss, img_class_loss, txt_class_loss), (img_auc, txt_auc)
...
files = np.unique(raw_data['files']).tolist()[:200]

device = 'cuda' if torch.cuda.is_available() else 'cpu'
model = SIGLIP_CLASSIFIER().to(device)
...
dataset_dir = '/data/cl2920/MIMIC-CXR/mimic-cxr-jpg-2.0.0.physionet.org/'
images = []
emb_list = []
for file in tqdm(files):
    dir = os.path.join(dataset_dir, file)  # Ensure proper path concatenation
    img = skimage.io.imread(dir)
    ...
    # Perform inference
    with torch.no_grad():
        encoded_images = model.vision_encoder(img_tensor)
        emb = model.vision_projection(encoded_images)

    # Convert embeddings to numpy and store in list
    emb_list.append(emb.cpu().numpy().tolist())
    
output_dict = {'files': files, 'embeddings': emb_list}
    
with open('src/train/utils/embs_14_siglip_classification.json', 'w') as file:
